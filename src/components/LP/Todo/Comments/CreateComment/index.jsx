...
const CreateComment = ({ setWriteComment, todo, addComment }) => {
  ...
  const submitComment = async (data) => {
    ...
    try {
      const response = await server.post(`${apiBaseUrl}/todo/${todo?.id}/comments`, {...data, userId: activeToken?.id, todoId: todo.id, priority: todo?.priority ?? ''})
  ...
  return (
    <Box as="form" onSubmit={handleSubmit(submitComment)} p={4} my={2} fontFamily="arial">
      ...
      <Flex gap={2}>
        <Button variant="primary" type="submit" bg="#5b60e9" fontSize="sm" textTransform="capitalize" color="white">
          Comment
        </Button>
